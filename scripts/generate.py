...
def signal_svg(config: dict, stats: dict) -> str:
    ...
    metric_nodes = []
    colors = [RED, ORANGE, GOLD, "#ff6b50"]
    for index, (label, value) in enumerate(metrics):
        x = 31 + index * 212
        try:
            bar_val = int(float(''.join(c for c in str(value) if c.isdigit() or c == '.') or '0'))
        except (ValueError, TypeError):
            bar_val = 0
        bar_len = 12 + min(33, bar_val % 34)
        stroke_color = colors[index % len(colors)]
        metric_nodes.append(f'<g transform="translate({x} 65)" class="reveal" style="animation-delay:{index * 0.09:.2f}s"><rect width="195" height="91" rx="15" fill="#140e0f" stroke="#4d2523"/><text x="16" y="26" class="label">{label}</text><text x="16" y="65" class="metric">{value}</text><path d="M132 67h45" stroke="#41201e" stroke-width="5" stroke-linecap="round"/><path d="M132 67h{bar_len}" stroke="{stroke_color}" stroke-width="5" stroke-linecap="round"/></g>')
    ...
    for index, (language, count) in enumerate(stats["languages"][:5]):
        ...
        lang_nodes.append(f'<text x="50" y="{y}" class="body strong">{escape(language)}</text><text x="164" y="{y}" class="mono">{round(count / total * 100)}%</text><rect x="210" y="{y - 11}" width="278" height="8" rx="4" fill="#30191a"/><rect x="210" y="{y - 11}" width="{width}" height="8" rx="4" fill="{color}" class="bar" style="animation-delay:{index * 0.08:.2f}s"/>')
